...
### 블록체인
  - 블록체인이란
    ...
    - 블록: 관리 대상 데이터를 저장하는 자료구조
      - 데이터의 종류: 거래에 관련된 사람들(구매자, 판매자)의 정보, 회사간의 계약 내용, 금융 거래 내역(돈을 보낸 자, 받은 자) 등
  ...
  - 트랜잭션 과정
    - 유저 A가 트랜잭션 요청 -> 해당 데이터 블록에 저장됨
    - 해당 블록은 네트워크 내 다른 유저들에게 전달되어 각 유저들이 거래의 타당성을 확인
    - 거래에 이상이 없다면 새로운 블록 데이터가 생성됨 -> 기존의 블록체인에 연결됨 -> 트랜잭션 종료
